...
void GameObject3D::UpdateDirectionVectors()
{
	...
	XMMATRIX vecRotationMatrixNoY = XMMatrixRotationRollPitchYaw(0.0, rot.y, 0.0);
	vec_forward_noY = XMVector3TransformCoord(DEFAULT_FORWARD_VECTOR, vecRotationMatrixNoY);
	vec_backward = XMVector3TransformCoord(DEFAULT_BACKWARD_VECTOR, vecRotationMatrixNoY);
